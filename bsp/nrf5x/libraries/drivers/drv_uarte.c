                              ...
                              void *                     p_context)
{
    ...
    switch (p_event->type)
    {
        case NRFX_UARTE_EVT_RX_DONE:
            ...
            //(void)nrfx_uarte_rx(&(uarte_instance_te), p_cb->rx_buffer, 1);
            /* Provide new UARTE RX buffer. */
		    nrfx_uarte_rx_enable(&(uarte_instance_te), 0);
            break;
        case NRFX_UARTE_EVT_RX_BUF_REQUEST:
		nrfx_uarte_rx_buffer_set(&(uarte_instance_te), p_cb->rx_buffer, 1);
		//buf_idx++;
...
static rt_err_t _uart_cfg(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    if(p_cb->isInit)
    {
        nrfx_uarte_uninit(&(uarte_instance_te));
        p_cb->isInit = false;
    }
   ...
   //                         NRF_UARTE_PARITY_EXCLUDED:NRF_UARTE_PARITY_INCLUDED;
    ...
    err = nrfx_uarte_init(&(uarte_instance_te),&config,uarte_evt_handler);
    if (err != NRFX_SUCCESS) {
		return RT_ERROR;
	}
    //nrfx_uarte_rx(&(uarte_instance_te),p_cb->rx_buffer,1);
    /* Start reception */
	err = nrfx_uarte_rx_enable(&(uarte_instance_te), 0);
	if (err != NRFX_SUCCESS) {
...
static int _uart_putc(struct rt_serial_device *serial, char c)
{
    ...
    p_cb->tx_buffer[0] = c;
    nrfx_uarte_tx(&(uarte_instance_te),p_cb->tx_buffer,1,NRFX_UARTE_TX_BLOCKING);
    if (err != NRFX_SUCCESS) {
				return RT_ERROR;
			}
/*    if(!(serial->parent.open_flag&RT_DEVICE_FLAG_INT_TX))
    {
        while(nrfx_uarte_tx_in_progress(&(uarte_instance_te)))
        {
        }
    }*/
